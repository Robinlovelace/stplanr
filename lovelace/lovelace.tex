...
\textbf{stplanr} facilitates common transport planning tasks including:
...
\CRANpkg{sp}, \CRANpkg{rgeos} and \CRANpkg{rgdal} greatly improved R's spatial abilities \citep{bivand_applied_2013}, work that is being consolidated and extended in the recent \CRANpkg{sf} package.
...
% \textbf{stplanr} aims to reduce barriers of access to such datasets planning, financial and technical, through open source software.
...
\textbf{stplanr} imports both \CRANpkg{sp} and its successor \CRANpkg{sf}.
...
Furthermore, output objects of class \texttt{sf} are *mostly* compatible with the popular data processing package \CRANpkg{dplyr}.
...
handle objects of class \texttt{sf} from the new \CRANpkg{sf} package
...
\textbf{tigris} and \CRANpkg{osmdata}
...
which creates an object named \texttt{l}, a spatial object that will be visualised
in the next section:
...
The code chunk below makes use of the function \texttt{geo\_buffer}, and highlights \CRANpkg{stplanr}'s ability to work with \texttt{Spatial} or \texttt{sf} objects interchangeably.
Figure \ref{fig:flights} demonstrates how the resulting spatial object can be plotted interactively using packages such as \CRANpkg{tmap} (see the accompanying R file for visualisation code).
...
API key
  ...
  Note: an updated interface to cyclestreets.net has been made available via the package \CRANpkg{cyclestreets}.}
...
using the \texttt{readOGR} function from \CRANpkg{rgdal}.
...
\CRANpkg{stplanr} focuses on geographic analysis of transport systems, a niche that is apparent from existing tools
...
The recent development of the \CRANpkg{sf}, with its support for multiple geometry columns, makes this option more feasible.
...
\CRANpkg{stplanr} was first developed to generate data for the Propensity to Cycle Tool (PCT), which estimates cycling potential down to the street level across all major cyclable routes in England and Wales \citep{lovelace_propensity_2017}.
