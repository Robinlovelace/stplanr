...
The \pkg{stplanr} package demonstrates and provides a home for such tools, with an emphasis on spatial transport data and non-motorized modes.
The \pkg{stplanr} package facilitates common transport planning tasks including:
downloading and cleaning transport datasets;
creating geographic ``desire lines'' from origin-destination (OD) data;
route assignment, locally and
interfaces to routing services such as \url{CycleStreets.net};
...
datasets.
More broadly, the experience of developing and using R functions for transport applications
shows that open source software can form the basis of a reproducible
transport planning workflow.
The \pkg{stplanr} package, alongside other packages and open source projects,
could provide a more transparent and democratically accountable alternative to the
current approach, which is heavily reliant on proprietary and relatively inaccessible software.
}
...
Transport planners have always undertaken a wide range of computational activities \citep{boyce_forecasting_2015}, but with the digital revolution the demands have grown beyond the capabilities of a single, monolithic product.
The diversity of tasks, and the need for democratic accountability in public decision making, suggests that future-proof transport planning software should be:
...
The \CRANpkg{sp}, \CRANpkg{rgeos}, and \CRANpkg{rgdal} packages greatly improved R's spatial abilities \citep{bivand_applied_2013}, work that is being consolidated and extended in the recent \CRANpkg{sf} package.
...
The purpose of \pkg{stplanr} is to provide a toolbox rather than a specific solution for transport planning, with an emphasis on spatial data and active modes.
...
% \pkg{stplanr} could be extended in non-spatial directions such as mode
% choice modelling.
...
% \pkg{stplanr} aims to reduce barriers of access to such datasets planning, financial and technical, through open source software.
...
% The paper is therefore not only about showcasing new tools within \pkg{stplanr} but also about demonstrating how \emph{existing} tools in R can be used for transport planning.
%
% \pkg{stplanr} cannot cover all bases in Transport Planning and it is hoped that the ideas, functions and datasets described in this paper inspire
...
% \pkg{stplanr}'s functionality over time
...
schema for classifying the kinds of task that \pkg{stplanr} can tackle:
...
software systems, as illustrated by the use of \pkg{leaflet}
...
(\pkg{stplanr} was peer reviewed thanks to the community surrounding ROpenSci).
...
visualisation packages (e.g. \pkg{tmap}, \pkg{ggplot2}),
...
\pkg{stplanr} imports both \pkg{sp} and its successor \pkg{sf}.
This means that spatial objects used in and produced by the package
work with base R generic functions such as
\code{summary}, \code{aggregate}, and \code{plot} \citep{bivand_applied_2013}.
Furthermore, output objects of class \code{"sf"} are \textit{mostly} compatible with the popular data processing package \CRANpkg{dplyr}.
...
\item
  ...
  geographical space. An example is provided in the \code{flow}
  dataset.
\item
...
\item
  ...
  \code{route\_} function. Route network represent many overlapping
  routes. All are typically stored as a \code{"SpatialLinesDataFrame"}
  object.
\end{itemize}

A convention has been developed whereby function names are
prefixed depending on the the input data type
(\code{od\_}, \code{line\_} and
\code{route\_} respectively, although \code{route\_} functions do not take routes as inputs, they output them). A selection of these is presented in
Table 1 (\code{lsf.str("package:stplanr")} returns a list of all functions).
Additional ``core functions'' could be developed, such as those
prefixed with \code{rn\_} (for working with route network data) and
\code{geo\_} functions for geographic operations such as buffer creation
...
\code{line2route}, for example, takes spatial lines objects and returns spatial lines objects.
Type stability has its limitations with spatial data, however:
it would be wasteful for functions such as \code{line\_bearing} (which returns the bearing of a line) to duplicate the spatial data contained in its input, for instance.
Generic classes enable \pkg{stplanr} to
handle objects of class \code{"sf"} from the new \pkg{sf} package
...
description of each of \pkg{stplanr}'s functions, the remainder of this paper shows how the package can be used for transport planning, beginning with data access and ending with visualisation.
...
\pkg{tigris} and \CRANpkg{osmdata}
\citep{walker_tigris:_2016}.

The \pkg{stplanr} package helps import data with functions including
\code{read\_table\_builder}, for importing data from the
Australian Bureau of Statistics (ABS), and
\code{dl\_stats19} --- which has now been split-out into the package \CRANpkg{stats19} --- for downloading datasets from the UK's Stats19 road traffic
...
\code{tempdir}, it can be read-in and cleaned with the
\code{read\_stats19\_} functions (note these call
\code{format\_stats19\_} functions internally to clean the datasets
...
Note the \code{inner\_join} function from the \pkg{dplyr} package was used because it is substantially faster and more effective than the equivalent function, \code{merge}, in base R.
This is communicated in a number of places, including on the website \href{http://zevross.com/blog/2014/04/30/mini-post-for-large-tables-in-r-dplyrs-function-inner_join-is-much-faster-than-merge/}{zevross.com}. For this reason we import \pkg{dplyr} and use it internally for joins.}
...
\pkg{stplanr}. The following code, for example, geographically
...
network dataset} contained as a dataset in \pkg{stplanr} for illustrative purposes.
Note the use of \code{bb2poly}, which converts a spatial
...
These can be visualised using base R graphics, extended by \pkg{sp},
...
\caption[Road traffic fatalities in the study area downloaded with with \pkg{stplanr} (crosses)]{Road traffic fatalities in the study area downloaded with with stplanr (crosses). Deaths that happened within 100 m of the route network are represented by circles.}\label{fig:fats}
\end{figure}
\end{Schunk}
...
The \pkg{stplanr} package provides the \code{read\_table\_builder} function to read in these files.

Using the example \texttt{SA1Population.xlsx} file included with \pkg{stplanr} that contains the population by SA1 zone (a statistical area used for the Australian census), we can use the \code{read\_table\_builder} function to read and format the table for use in R.
...
The \pkg{stplanr} package can also be used in conjunction with complementary R packages for downloading data from Open Street Maps (OSM) using \pkg{osmdata} and bicycle share data using the \CRANpkg{bikedata} package.

The bicycle share data that can be accessed using the \pkg{bikedata} package is particularly well suited for integration with \pkg{stplanr} as it produces origin-destination (OD) flows from bicycle sharing systems.
This data can be used together with the \code{sum\_network\_links} function to generate the likely paths and how these overlap.
...
the \code{flow} object.
\code{flow} is a data frame representing the number
...
(see \code{?flow} to see how this dataset was created):
...
separate geographical dataset (represented by the \code{cents} dataset),
as illustrated below:\footnote{Such geographic datasets are best represented as in a spatial class system, explaining
\pkg{stplanr}'s close integration with R's spatial packages.}
...
A common task is linking an OD dataset (e.g., \code{flow}) to a geographic dataset
representing zone centroids (e.g. \code{cents}).
We use \code{od2line} to combine them, as illustrated in the code chunk below,
which creates an object named \code{l}, a spatial object that will be visualised
in the next section:
...
\begin{figure}
\includegraphics[width=\textwidth]{flights} \caption[Flights from New York]{Flights from New York to airports within 1000 km. Data from the \CRANpkg{nycflights} package converted to geographic desire lines with the \code{od2line} function.}\label{fig:flights}
\end{figure}

A larger example represents flights from New York in 2013 from the \CRANpkg{nycflights13} package.
The code chunk below makes use of the function \code{geo\_buffer}, and highlights \pkg{stplanr}'s ability to work with \code{"Spatial"} or \code{"sf"} objects interchangeably.
Figure \ref{fig:flights} demonstrates how the resulting spatial object can be plotted interactively using packages such as \pkg{tmap} (see the accompanying R file for visualisation code).
...
The \pkg{stplanr} package tackles this issue in two ways: by providing routing
...
These allocate a single OD pair (represented by \code{from} and \code{to}
arguments as coordinates,
spatial point objects, or text strings to be
"geo-coded") to the transport network.
This is illustrated
below with \code{route\_cyclestreet}, which uses the
...
API key
  ...
  See \code{?route\_cyclestreet} for details. Thanks to Martin
  Lucas-Smith and Simon Nuttall for making this possible.
  Note: an updated interface to \url{cyclestreets.net} has been made available via the package \CRANpkg{cyclestreets}.}
...
GeoJSON text string. By default, \code{route\_cyclestreet} saves a
...
\code{save\_raw} argument can be used:
...
Additional arguments taken by the \code{route\_} functions depend on
the routing function in question. By changing the \code{plan} argument
of \code{route\_cyclestreet} to \code{fastest}, \code{quietest}, or
\code{balanced}, for example, routes favouring speed, quietness or a
...
lines, the \code{line2route} function loops over each line, wrapping
any \code{route\_} function as an input. The output is a
\code{SpatialLinesDataFrame} with the same number of dimensions as the
...
functionality (see \code{vignette("introducing-stplanr")} and \href{http://kateto.net/network-visualization}{kateto.net} for more sophisticated visualisation methods).
...
transport network, the \code{overline} function demonstrated above, is
...
Limitations with the \code{route\_cyclestreet} routing API include its
...
added with the functions \code{route\_graphhopper},
\code{route\_transportapi\_public}, and \code{viaroute}. These
...
and Oaxaca demonstrates how \code{route\_graphhopper} can collect
...
The \pkg{stplanr} package provides functionality that calculates catchment areas using
straight-line distances with the \code{calc\_catchment} function. This
function takes a \code{"SpatialPolygonsDataFrame"} object that contains the
population (or other) data, typically from a census, and a
\code{Spatial*} layer that contains the geometry of the transport
...
using the \code{readOGR} function from \pkg{rgdal}.
The resulting object \code{smallsa1} contains population data for Statistical Area 1
(SA1) zones in Sydney, Australia. The
\code{testcycleway} data set contains hypothetical
...
The calculation of catchment areas requires two parameters:
...
catchment areas of the cycleway, with the command \samp{sum(catch800m\$Total)}: 39418 people.
...
more realistic catchment areas for most situations, \pkg{stplanr}
provides the \code{calc\_network\_catchment} function that uses the
same principle as \code{calc\_catchment} but also takes into account
the transport network.

To use \code{calc\_network\_catchment}, a transport network,
as a \code{"SpatialLinesNetwork"} object, must be provided. This combines a
\code{"SpatialLinesDataFrame"} object with a graph network (using the
\CRANpkg{igraph} package) to allow routing (estimation of shortest paths).
...
\code{calc\_catchment} but with a few minor changes. Specifically
these are including the \code{SpatialLinesNetwork}, and using the
\code{maximpedance} parameter to define the distance, with distance
...
the \code{calc\_catchment} and \code{calc\_network\_catchment}
functions, the \code{maximpedance} parameter is the maximum value in
...
Although for many transport applications it would be better to use the \code{calc\_network\_catchment} function that uses the true network distances, it may still be useful to use the straight-line version in some cases.
...
In situations where network data is not available it would be possible to use the \code{calc\_catchment} function together with a fixed assumption about what straight-line distance corresponds to the desired network distance (i.e., 1km network = 700m straight-line).
...
generated by \pkg{stplanr} can be used to undertake such analysis,
with the \code{line\_length} function used to find the distance, in
...
\code{route\_cyclestreet}, we can estimate average values for both
metrics as follows:
...
The \pkg{stplanr} package is not intended primarily as an SIM package,
but could be extended in this direction, as illustrated by the \code{od\_radiation} function, which implements a SIM proposed by \citet{simini_universal_2012}.
Further development in this direction (or integration with a package dedicated to SIMs) would complement \pkg{stplanr}'s focus on geographic functions.
...
trips made by walking using the same object \code{l} generated by
\pkg{stplanr}.
...
\code{summary()}. Surprisingly, Euclidean distance was a better
...
possibilities created by using \pkg{stplanr} in conjunction with R's
...
surprise that \pkg{stplanr} contains no functions for visualisation.
Instead, users are encouraged to make use of existing spatial
visualisation tools in R, such as \pkg{tmap}, \pkg{leaflet}, and
\pkg{ggmap} \citep{cheshire_spatial_2015,kahle_ggmap:_2013}.

Furthermore, with the development of online application frameworks, such
as \pkg{shiny}, it is now easier than ever to make the results of
...
The \pkg{stplanr} package focuses on geographic analysis of transport systems, a niche that is apparent from existing tools.
...
The lack of existing packages that do the same job direct our attention towards \pkg{stplanr}'s basic functionality rather than more advanced features.
A good example of this is the \code{SpatialLinesNetwork} class which could provide a basis for future developments including the recently implemented \code{route\_local} function.
...
The recent development of the \pkg{sf}, with its support for multiple geometry columns, makes this option more feasible.

Another direction of travel is towards better support for large datasets.
We have demonstrated \pkg{stplnr}'s capabilities on deliberately small datasets for ease of understanding.
...
Some functions use C++ (via \CRANpkg{Rcpp}) for computationally intensive operations, and we plan to continue the drive toward performant code.
...
integration with transport databases (e.g., via the \pkg{bikedata} package);
...
% `reinvent the wheel' (e.g.,~with a new class system), \pkg{stplanr}
% builds on existing packages and \pkg{sp} classes to work with common
% transport data formats.

Rather than a one-size-fits-all package, we see \pkg{stplanr} as an additional tool in the transport planner's cabinet.
...
In this context, \pkg{stplanr}'s focus on
...
sources.
The \pkg{stplanr} package could also be used alongside other R packages that
use spatial transport data such as \CRANpkg{aspace} and
\CRANpkg{MCI} \citep{RJ-2017-020}.

The \pkg{stplanr} package was first developed to generate data for the Propensity to Cycle Tool (PCT), which estimates cycling potential down to the street level across all major cyclable routes in England and Wales \citep{lovelace_propensity_2017}.
...
% \pkg{stplanr} occupies an important niche in the transport planning
% software landscape, with its focus on spatial transport data. There is
% great potential for development of \pkg{stplanr} in many directions.
...
% databases. Our priority for \pkg{stplanr} however, is to keep the
...
%   integration with the in-development \pkg{osmdata} package.
% \item
%   Functions to match different GIS routes, perhaps building on the
%   Hausdorf distance algorithm implemented in the \pkg{rgeos}
%   function \code{gDistance}.
% \item
...
%   migration to \pkg{sf} for spatial classes.
% \end{itemize}
%
% Such spatial data processing capabilities would increase the range of
% transport planning tasks that \pkg{stplanr} can facilitate. For all
...
% in \pkg{stplanr} will go into improving documentation and
% user-friendliness.
%
% We have set out clear motivations for
% developing transport planning capabilities in R and believe that
% \pkg{stplanr} provides major a step in that direction.
...
examples of this are the \pkg{osmdata} CRAN package \citep{Padgham2017} and the Python package
...
packages for analyzing transport datasets. The \pkg{stplanr} package could help
ensure that the
...
We would like to thank a number of people and organisations who have supported the development of \pkg{stplanr}:
the UK's Department for Transport (DfT) for funding the Propensity to Cycle Tool (see \href{http://www.pct.bike/}{www.pct.bike}),
which instigated code which eventually became \pkg{stplanr};
