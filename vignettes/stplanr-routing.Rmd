...
# saveRDS(r_osrm, "vignettes/r_osrm.Rds")
```
...
We can compare these different routes, as illustrated in Figure \@ref(fig:routes):
...
osm_jena = opq(bbox = bb_jena) %>% 
  add_osm_feature("highway", "prim|second", value_exact = FALSE) %>% 
  osmdata_sf()
summary(osm_jena)
ways = osm_jena$osm_lines
# saveRDS(ways, "vignettes/ways.Rds")
```

To save time we've saved the result, which can be loaded as follows from the vignettes folder:

```{r}
ways = readRDS("ways.Rds")
```


Let's check if this is a good route network representing Jena:
